...
def evaluation_and_performance(test_configuration:dict,
    ...
    other_top_perf=test_configuration['other_top_perf']
    for el in other_top_perf:
        results_perf['@'+str(el)]={'microf1':fn_micro(test_y_true[top_indices[:el]], test_y_pred[top_indices[:el]]),
                                   'macrof1':fn_macro(test_y_true[top_indices[:el]], test_y_pred[top_indices[:el]])}
...
def process_train_batch_kNN_weighted(train_batch_x, 
                            train_batch_y,
                            test_batch_embedding, 
                            model_weights,
                            test_configuration,
    ...
    n_jobs = test_configuration['n_jobs']
    eval_method = test_configuration['eval_method']
    class_probabilities = test_configuration['class_probabilities']  # Probabilità delle classi dal set di training
    ...
    # Calcoliamo gli embeddings del batch di training
    train_batch_embedding = model.predict(train_batch_x, verbose=0)

    # Adattiamo il modello kNN utilizzando gli embeddings
    nbrs = NearestNeighbors(n_neighbors=kNN, algorithm=algo, metric=distance, n_jobs=n_jobs).fit(train_batch_embedding)
    
    # Troviamo le distanze e gli indici dei k-nearest neighbors
    distances_embedding, indices = nbrs.kneighbors(test_batch_embedding)

    # Applichiamo la pesatura con softmax basata sulla distanza
    weights = np.exp(-distances_embedding / temperature)
    ...
    if eval_method=='multi':
        if test_configuration['get_thresholds']:
            weightsN=((1-train_batch_y.numpy()[indices, :])*np.expand_dims(weights, -1)).sum(1)*(1-np.expand_dims(class_weights,0))
        else:
            print('likelihood no')
            weightsN=np.sum(np.expand_dims(weights, -1) * (1-train_batch_y.numpy())[indices, : ] , 1)
        weightsPN=np.concatenate([np.expand_dims(weightsP, 0),np.expand_dims(weightsN, 0)],0)
...
def chunk_kNN_ml_prediction(training_dataset:tf.data.Dataset,
    ...
    print('model kNN ml prediction', sum([el.size for el in model_weights]))
    ...
    if test_configuration['calibrated']:
        threshold = test_configuration['class_probabilities']
    else:
        threshold = test_configuration['threshold']
    ...
    for test_batch_x, test_batch_y in test_dataset.batch(val_batch):
        ...
        R=test_batch_y.numpy().sum(1)
        pred_at_R_batch=compute_one_hot(norm_w_batch, R)
        
        
        if test_configuration['calibrated']:
            y_pred_batch = (norm_w_batch>np.expand_dims(threshold,0)).astype(int)
            if test_configuration['harmonic_score']:
                test_y_score_batch = np.where(y_pred_batch.sum(1)>0,
                                            np.where(norm_w_batch>=np.expand_dims(threshold,0),
                                                    norm_w_batch,1).prod(1)**(1./(y_pred_batch.sum(1)+1e-20))
                                                    ,0.)
            else:
                test_y_score_batch = np.where(norm_w_batch>=np.expand_dims(threshold,0),norm_w_batch,0).sum(1)/((y_pred_batch).sum(1)+1e-20)

        else:
            y_pred_batch = (norm_w_batch>=threshold).astype(int)
            if test_configuration['harmonic_score']:
                test_y_score_batch = np.where(y_pred_batch.sum(1)>0,
                                          np.where(norm_w_batch>=threshold,
                                                   norm_w_batch,1).prod(1)**(1./(y_pred_batch.sum(1)+1e-20))
                                                   ,0.)
            else:
                test_y_score_batch = np.where(norm_w_batch>=threshold,norm_w_batch,0).sum(1)/((y_pred_batch).sum(1)+1e-20)
...
# GPU funzionante
# Define the parallelizable function for the inner loop
def process_train_batch_kNN_GPU(train_batch_x, 
                            train_batch_y,
                            test_batch_embedding, 
                            #model_weights,
                            model,
    ...
    # Compute train batch embeddings using the model
    train_batch_embedding = model.predict(train_batch_x, verbose=0)

    # Use cuML NearestNeighbors instead of sklearn
    if distance == 'euclidean':
        metric = 'euclidean'
    elif distance == 'cosine':
        metric = 'cosine'
    else:
        metric = 'l2'  # Defaulting to L2 distance
    
    nbrs = cuMLKNN(n_neighbors=kNN, metric=metric)
    nbrs.fit(train_batch_embedding)
...
def chunk_kNN_ml_prediction_GPU(training_dataset: tf.data.Dataset,
                            test_dataset: tf.data.Dataset,
                            model: tf.keras.models,
                            test_configuration: dict):
    
    #Modified version that uses cuML for kNN search.
    ...
    with strategy.scope():
        ...
        if distance == 'cosine_triangle':
            ...
            algo = 'auto'  # Use cuML optimized methods
        ...
        for test_batch_x, test_batch_y in test_dataset.batch(val_batch):
            ...
            print("Iteration in test batch number: "+str(counter_test)+" of "+str(len(test_dataset.batch(val_batch))))
            ...
            counter_trrain = 0
            for train_batch_x, train_batch_y in training_dataset.batch(val_batch):
                print("Iteration in train batch number: "+str(counter_trrain)+" of "+str(len(training_dataset.batch(val_batch))))
                weights = process_train_batch_kNN_GPU(train_batch_x, 
                                                train_batch_y, 
                                                test_batch_embedding, 
                                                #model_weights, 
                                                model,
            ...
            norm_w_batch = cp.asarray(norm_w_batch)

            
            y_pred_batch = (norm_w_batch >= threshold).astype(cp.int32)
            test_y_score_batch = cp.where(norm_w_batch >= threshold, norm_w_batch, 0).sum(1) / (y_pred_batch.sum(1) + 1e-20)

            test_y_pred.append(cp.asnumpy(y_pred_batch))
            test_y_score.append(cp.asnumpy(test_y_score_batch))
            test_y_true.append(test_batch_y.numpy())
            counter_test += 1
        ...
        top_indices = np.argsort(-test_y_score)[:test_configuration['top_perf']]
...
def compute_one_hot(matrix, array):
    one_hot_encoded = []
    for i, n_i in enumerate(array):
        row = matrix[i]
        # Get the indices of the top-n_i values
        top_indices = np.argsort(row)[-n_i : ]  # Get indices of top n_i elements
        # Create a one-hot encoded vector for the row
        one_hot = np.zeros_like(row, dtype=int)
        one_hot[top_indices] = 1
        one_hot_encoded.append(one_hot)
    return np.array(one_hot_encoded)

def precision_at_R_ml(y_true, pred_at_R):
    R=y_true.sum(1)
    tp = ((y_true == 1) & (pred_at_R == 1)).sum(1)

    prec_at_R=np.where(R!=0,tp/R,np.nan)
    prec_at_R=np.nanmean(prec_at_R)
    return prec_at_R
...
def f1_macro_yt_ml(y_true, y_pred):
    ...
    for i in range(y_true.shape[1]):
        ...
        # Considera solo se ci sono esempi positivi (evitare divisioni per zero)
        if tp + fn + fp > 0:
            f1_macro.append(tp / (tp + 0.5 * (fn + fp)))
        #else:
        #    f1_macro.append(1)
    
    return np.nanmean(f1_macro)



# #f1 macro multilabel old
# def f1_macro_yt_ml(y_true,y_pred):
#     f1_macro=[]
#     for i in range(y_true.shape[1]):
...
def train_one_grid_point(gs_point:list, output_path:str, cols:list, 
    ...
    configuration = {
                     ...
                     'optimizer':tf.keras.optimizers.serialize(tf.keras.optimizers.AdamW(), use_legacy_format=True),
                     'epochs':100,
                     'val_batch':100000,
                     'n_jobs':-1,
                     'top_perf':1000,
                     'eval_method':'multi', #'unroll', 'isin',
    ...
    for key,entry in zip(cols,gs_point):    
        if key in ['learning_rate', 'temperature','pert','jaccard_threshold']:
            entry=float(entry)
        elif key in ['output_dimensions','depth', 'batch_size','kNN','input_shape']:
            try:
                entry=int(entry)
            except ValueError:
                entry=bool(entry)
    ...
    for value in [True,False]:
        configuration['get_thresholds']=value
        if validation:
            test_y_true, test_y_pred, test_y_score, results_perf, top_indices = evaluation_and_performance(test_configuration=configuration,
                                                                                                            training_dataset=training_dataset,
