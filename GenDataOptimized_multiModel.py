...
def load_wiki20m(path, file_name, columns, NA=False):
    raw_data = []
    clean_text = []
    clean_text_map = {}  # Dictionary to map text to its index
    processed_data = []
    ...
    for doc in raw_data:
        ...
        # Use a dictionary for O(1) lookup instead of list.index (O(n))
        ...
        # Precompute token boundaries once per document
        token_boundaries = []
        current_pos = 0
        for token in doc['token']:
            token_len = len(token)
            token_boundaries.append((current_pos, current_pos + token_len))
            current_pos += token_len + 1  # +1 for the space

        # Head position calculation
        head_tok_start, head_tok_end = doc['h']['pos']
        head_start_bound = token_boundaries[head_tok_start][0]
        head_end_bound = token_boundaries[head_tok_end - 1][1]

        # Tail position calculation
        tail_tok_start, tail_tok_end = doc['t']['pos']
        tail_start_bound = token_boundaries[tail_tok_start][0]
        tail_end_bound = token_boundaries[tail_tok_end - 1][1]
...
def init_Word2Vec(only_real_token=False, extract_CLS_only=False):
    """Initialize FastText model (Facebook's improved Word2Vec with subword support)"""
    ...
    class FastTextTokenizer:
        ...
        def __call__(self, text):
            ...
            # First split into sentences, then tokenize each sentence
            sentences = self.sent_tokenizer.tokenize(text)
            tokens = []
            for sent in sentences:
                tokens.extend(self.word_tokenizer(sent.lower()))
            return tokens

    # 1. Load pre-trained FastText model (automatically downloads on first run)
    model_path = '/home/lucamariotti/gensim-data/fasttext-wiki-news-subwords-300/fasttext-wiki-news-subwords-300.gz'
    
    # 2. Download synchronously before parallel execution
    # Verify model exists
    if not os.path.exists(model_path):
        raise FileNotFoundError(
            ...
            "unzip wiki-news-300d-1M.vec.zip -d ~/gensim-data/"
        )
    ...
    class FastTextEmbeddingModel(tf.keras.Model):
        def __init__(self, kv_model):
            super(FastTextEmbeddingModel, self).__init__()
            self.kv_model = kv_model
            self.vector_size = kv_model.vector_size
            self.tokenizer = FastTextTokenizer()

        def call(self, inputs):
            # Convert tensor inputs to text
            ...
            for text in texts:
                ...
                for token in tokens:
                    try:
                        vec = self.kv_model[token]
                    except KeyError:
                        # Handle OOV words with subword information
                        vec = np.zeros(self.vector_size)
                        ngrams = [token[i:i+3] for i in range(len(token)-2)] if len(token) >= 3 else [token]
                        found = 0
                        
                        for ng in ngrams:
                            try:
                                vec += self.kv_model[ng]
                                found += 1
                            except KeyError:
                                continue
                    ...
                    sentence_emb.append(vec)
                
                embeddings.append(np.stack(sentence_emb) if sentence_emb else 
                                np.zeros((1, self.vector_size)))
            
            return tf.convert_to_tensor(embeddings, dtype=tf.float32)

    tokenizer = FastTextTokenizer()
    return FastTextEmbeddingModel(ft_model), tokenizer, tokenizer
...
def init_RoBERTa(only_real_token=False, extract_CLS_only=False):
    ...
    tokenizer = RobertaTokenizer.from_pretrained(model_name, cache_dir="./roberta-tf")
    encoder = TFRobertaModel.from_pretrained(model_name, trainable=False, from_pt=False, cache_dir="./roberta-tf")
...
def find_entity_tensor_slice(frase, preprocessor, entity_start_pos, entity_end_pos):
    ...
    # Check for Word2Vec/FastText case
    is_word2vec = (
        preprocessor is None or 
        (hasattr(preprocessor, '__class__') and 'FastTextTokenizer' in str(preprocessor.__class__)))
    
    if is_word2vec:  # Word2Vec/FastText case
        tokens_to_end = word_tokenize(frase[:entity_end_pos].lower())
        entity_tokens = word_tokenize(frase[entity_start_pos:entity_end_pos].lower())
        bound_end = len(tokens_to_end)
        bound_start = bound_end - len(entity_tokens)
        # logger.info(f"Tokens: {tokens_to_end}")
        # logger.info(f"Entity tokens: {entity_tokens}")
        # logger.info(f"Bound start: {bound_start}, Bound end: {bound_end}")
    
    
    elif isinstance(preprocessor, RobertaTokenizer):  # RoBERTa case
...
def check_if_head_tail_are_included_in_Tokens(sentence, preprocessor, h_s, h_e, t_s, t_e, bert_token_window=512):
    ...
    if is_word2vec:  # Word2Vec/FastText case
        tokens = word_tokenize(sentence_to_index.lower())
...
def init_worker(model_name):
    ...
    if model_name == "bert":
        ...
        model, _, preprocessor = init_Word2Vec()
...
def preprocess_and_embed(dataset_name, model_name, NA=False):
    dataset_configs = {
        ...
        "disrex": ("RawDatasets/DiS-ReX/english/", "Datasets/DisRex/",
...
def main():
    ...
    parser.add_argument('--model', type=str, default='bert', choices=['bert', 'roberta', 'mamba','word2vec'], 
